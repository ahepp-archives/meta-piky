...
configure_ap () {
	...
	[ -x /usr/bin/autoAP-local.sh ] && /usr/bin/autoAP-local.sh AccessPoint
    fi
}
...
configure_client () {
	...
	logmsg "autoAP: Configuring $device as a Wireless Client"
        mv /etc/systemd/network/11-$device.network~ /etc/systemd/network/11-$device.network
        systemctl restart systemd-networkd
	[ -x /usr/bin/autoAP-local.sh ] && /usr/bin/autoAP-local.sh Client
    fi
}
...
# Main code
#
if [ -f /usr/bin/autoAP.conf ]
then
    source /usr/bin/autoAP.conf
else
...
then
    ...
    exec /usr/sbin/wpa_cli -i $2 -a /usr/bin/autoAP.sh 
    exit 0
fi
#
